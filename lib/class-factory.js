...
function ClassFactory (vm) {
  ...
  DexFile.fromBuffer = function (buffer) {
    ...
    const filePathValue = JFile.createTempFile('raw', 'dex', JFile.$new(cacheDir));
  ...
  DexFile.prototype = {
    ...
    getClassNames () {
      ...
      const optimizedDex = JFile.createTempFile('opt', 'dex', cacheDir);
        ...
        if (fn !== null) {
          ...
          try {
            ...
            withRunnableArtThread(vm, env, thread => {
              ...
              console.log('performed RequestDeoptimization');
            });
            console.log('<<<');
          } catch (e) {
